...
"""
    改进02 权重系数 固定权重->动态权重
    f=g+w*h      g:当前值       h：预估值
    w大，更相信预估值，速度快，但是可能会错过最优路径
    w小，更相信当前值，倾向于搜多最优路径，但是速度满了
    w过小，退化成Dijkstra(bfs)
    但是实际情况下，不能只考虑速度而不考虑路径，因此，要进行动态加权
"""
...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr):
        ...
        self.resolution = resolution  # grid resolution [m]分辨率
        self.rr = rr  # robot radius [m]
        self.calc_obstacle_map(ox, oy) # 构建膨胀后的栅格障碍物地图obmap
...
# def __str__ 相当于定义了结构体struct 并且进行打印输出（调用print打印节点时才能看出）
# 用于打印信息方便检查，此处为预留作用
        def __str__(self):
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        nstart = self.Node(self.calc_xyindex(sx, self.minx),  # position min_pos   2 (2.5)  # 把真实的世界坐标变为格子的编号
        ...
        while 1:
            ...
            #删除的是已经被用过的，从openList中删除，放入closeList中
            del open_set[c_id]
    ...
    # 领域，可以在此基础上改为更多或者更少的领域，也是A*的改进方法之一
    @staticmethod
